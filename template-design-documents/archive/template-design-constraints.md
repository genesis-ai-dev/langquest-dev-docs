...
| C2 | **Template-only (no template_node table)** | Template structure stored as JSONB in `template.structure`. Quests and assets reference `template_node_id` (opaque nanoid); the template is reached via the owning project (`project.template_id`). No materialized template_node records, no `project_template_link` table. |
...
| C6 | **One template per project** | A project has a single content structure: `project.template_id` (one template per project by construction — a single FK column, no link table). Multiple bodies of content are composed as subtrees within the single template (composition copies with fresh IDs). Side-by-side template trees were rejected — they act like sub-projects and add a layer to manage. |
...
| C8 | **Template sharing via templates** | Templates are shared as `template` rows. Projects reference templates via `project.template_id`. Forking = duplicating one row. |
...
| # | Constraint | Notes |
|---|-----------|-------|
| C11 | **Selective modification scope** | An editor can only apply template changes to projects *they own*, regardless of template ownership. Implemented via RPCs that fork a template and re-point `project.template_id`. |
...
| C14 | **Frozen project templates** | Pre-migration Bible/FIA projects have `template_frozen = true` on `project`, preventing re-pointing. The templates themselves remain forkable. |
...
| C21 | **Publish = insert a new immutable row** | Publishing always creates a new `template` row. In update mode, selected projects are re-pointed (`project.template_id` updated) to the new row. Provenance tracked via `copied_from_template_id`. |
...
| C24 | **No special app logic for Bible/FIA** | All UI driven generically by template structure fields (`node_type`, `linkable_type`, `contains_assets`, `allows_spanning`). `contains_assets` marks the quest that directly holds assets and serves as both version anchor and download unit. |
